...
class Model(object):
    ...
    def load_feature_map_model(self, t):
        ...
        self.feature_map_extractor_model = kerasModel(input=self.feature_map_extractor_model.input, output=self.feature_map_extractor_model.get_layer('flatten').output)
    ...
    def train_iterator(self, i, number_of_steps):
        ...
        if target_image.shape[0] <=1 or target_image.shape[1] <=1:
            print('pid:',pid,' img_path:',target_data["image"])
            return 0
        ...
        while step < number_of_steps:
            ...
            if len(self.replay) < self.buffer_experience_replay:
                ...
                self.rl_model.fit(X_train, y_train, batch_size=self.batch_size, epochs=1, verbose=0)
    ...
    def train_model(self, max_iters=10, number_of_steps=10):
        ...
        for i in range(max_iters):
            with timer('all', i):
                reward = self.train_iterator(i, number_of_steps)
            summary = tf.Summary()
